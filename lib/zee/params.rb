...
module Zee
  ...
  class Params
    ...
    # Convert the store to a hash.
    # @return [Hash] the store as a hash.
    def to_h
      @store
    end
    ...
    def require(key)
      ...
      unless @store.key?(key)
        raise ParameterMissingError, "param is missing: #{key}"
      end
    ...
    def permit(*keys)
      ...
      self.class.new(@store.slice(*keys))
    end
  end
end
